...
El propósito del proceso de diseño es definir una arquitectura que haga creíble y sostenible la promesa central de VotoChain: cada acuerdo de asamblea queda respaldado por votos de personas verificadas biométricamente, firmados individualmente y verificables por terceros sin depender de la directiva.

La problemática identificada muestra que el voto en asambleas de edificios multifamiliares depende de listas en papel, verificación manual de identidad y conteo a cargo de la propia directiva. Esto produce dificultad para comprobar cuórum real, riesgo de suplantación y voto múltiple, impugnaciones por falta de evidencia y exclusión de propietarios que no asisten presencialmente. El contexto agrava el problema: alrededor del 95% de edificios sin junta inscrita, 70% con conflictos de convivencia y morosidad cercana al 40% en juntas informales.

El diseño responde a los dos segmentos objetivo:

* **Segmento 1 - Directivas y administradoras:** necesitan convocar, verificar cuórum, cerrar votaciones y defender el resultado ante propietarios ausentes, SUNARP, municipalidades o conciliaciones. El diseño les da control operativo (crear comunidad, política, propuesta, padrón) sin permitirles alterar hechos congelados (`QuorumSnapshot`, `EligibilitySnapshot`, votos confirmados on-chain).
* **Segmento 2 - Propietarios / socios votantes:** necesitan votar remotamente desde su smartphone, con un flujo simple (selfie + elegir opción + comprobante), sin entender wallets, gas ni contratos, y con garantía de que nadie votó por ellos. El diseño oculta la complejidad criptográfica tras el modelo de wallet custodio "Modelo B" (derivación HD/BIP-32 por usuario, reconstrucción efímera de clave) y una wallet relayer que paga el gas.

El propósito se alinea con las hipótesis Lean UX H1–H4: confianza por verificación biométrica, reducción de impugnaciones por verificabilidad on-chain, aumento de cuórum por voto remoto y adopción por no-técnicos. Por ello, las decisiones arquitectónicas no se justifican por novedad tecnológica, sino por escenarios de calidad previsibles: seguridad de identidad, auditabilidad pública, privacidad de datos biométricos, desempeño en pico de asamblea y modificabilidad frente a proveedores externos.

El resultado estratégico ya anticipado es la descomposición en 11 Bounded Contexts con `Voting & Verifiable Ledger` como Core, apoyado por `Biometric Identity Verification`, `Cryptographic Wallet Custody`, `Blockchain Relay & Transaction Delivery`, `Membership`, `Community Management`, `Consent & Compliance`, IAM, Document OCR & Face Match Provider, Verification (OTP) y Notifications.

### **4.1.2. Attribute-Driven Design Inputs.**

Los inputs del diseño ADD son de tres tipos: funcionalidad primaria (historias con mayor impacto arquitectónico), escenarios iniciales de atributos de calidad y restricciones no-negociables. Se derivan del Capítulo III, del `Ubiquitous Language` y del Context Mapping (R1–R16).
...
| Epic / User Story ID | Título | Descripción | Criterios de Aceptación | Relacionado con (Epic ID) |
|---|---|---|---|---|
| US-19 | Examinar documento y rostro para el enrollment | Como propietario o socio, quiere validar su documento y rostro, para crear una referencia biométrica vinculada a su identidad. | **Dado** que la persona pertenece al menos a una comunidad y permite el tratamiento biométrico, **cuando** solicita el examen con evidencia válida, **entonces** el sistema extrae datos, compara rostros y produce un único veredicto `MATCH`, `NO_MATCH` o `UNREADABLE`. **Dado** que el examen concluye o vence, **cuando** se registra el veredicto, **entonces** elimina imágenes y artefactos temporales. **Dado** que el resultado es `NO_MATCH`/`UNREADABLE`/vencido, **cuando** solicita un nuevo examen, **entonces** se abre uno nuevo enlazado al anterior sin modificar su historial. | EP-06 |
| US-20 | Crear la referencia biométrica | Como propietario o socio, quiere completar su enrollment biométrico, para poder verificarse antes de cada voto. | **Dado** que existe consentimiento vigente y un `MATCH` válido no consumido, **cuando** solicita el enrollment, **entonces** se crea una única referencia biométrica sin almacenar documento ni selfies crudas. **Dado** que el `MATCH` fue consumido, venció o no pertenece a la persona, **cuando** solicita el enrollment, **entonces** se rechaza sin crear otra referencia. | EP-06 |
| US-21 | Verificar presencia e identidad antes del voto | Como propietario o socio elegible, quiere superar liveness y comparación facial, para obtener una autorización de voto de corta duración. | **Dado** que existe referencia biométrica y consentimiento vigentes, **cuando** inicia la verificación, **entonces** se evalúa primero liveness y solo después la comparación facial. **Dado** que ambas son satisfactorias, **cuando** concluye el intento, **entonces** se produce un veredicto `VERIFIED` con ventana de frescura. **Dado** que liveness falla, el rostro no coincide o el veredicto vence, **cuando** Voting consulta el estado, **entonces** se niega la condición de identidad verificada. | EP-06 |
| US-15 | Consultar padrón y determinar elegibilidad | Como directiva o administradora, quiere consultar el padrón y como propietario quiere conocer su elegibilidad, para participar solo cuando corresponde. | **Dado** que se consulta el padrón, **cuando** se obtienen las membresías vigentes, **entonces** se devuelven solo las que cumplen el estado activo definido. **Dado** que Voting solicita juzgar a una persona, **cuando** Membership evalúa su estado, **entonces** entrega un `EligibilitySnapshot` inmutable. **Dado** que la membresía cambia después, **cuando** se consulta la autorización otorgada, **entonces** no se reescribe la elegibilidad congelada. | EP-04 |
| US-23 | Abrir y consultar propuestas | Como directiva o administradora, quiere abrir una propuesta y como miembro quiere consultar las propuestas disponibles, para participar bajo reglas conocidas. | **Dado** que la comunidad está activa y tiene política vigente, **cuando** se abre una propuesta en borrador, **entonces** se congela la política como `QuorumSnapshot` y la propuesta queda abierta. **Dado** que un miembro consulta propuestas, **cuando** filtra por estado, **entonces** se devuelven solo las de su comunidad. **Dado** que la comunidad no admite nuevas propuestas, **cuando** se intenta abrir una, **entonces** se rechaza. | EP-07 |
| US-24 | Obtener autorización de voto | Como propietario o socio, quiere solicitar autorización para una propuesta, para votar solo si cumple identidad y elegibilidad. | **Dado** que la propuesta está abierta, la persona es elegible y posee veredicto biométrico fresco, **cuando** solicita autorización, **entonces** se concede una autorización breve, específica y de un solo uso. **Dado** que alguna condición no se cumple, **cuando** la solicita, **entonces** se deniega con razón verificable sin crear capacidad de firma. **Dado** que la autorización vence o fue consumida, **cuando** se intenta usar, **entonces** se rechaza como inválida o replay. | EP-07 |
| US-25 | Emitir un voto sin gestionar una wallet | Como propietario o socio autorizado, quiere elegir una opción y emitir su voto, para participar sin administrar claves ni pagar gas. | **Dado** que la autorización está vigente y la opción pertenece a la propuesta, **cuando** emite el voto, **entonces** se construye el contenido, se solicita su firma individual y se consume la autorización atómicamente. **Dado** que la firma se obtiene, **cuando** se registra el voto off-chain, **entonces** se marca como intención (`VoteCast`) pendiente de entrega y no se suma al resultado. **Dado** que falla la firma o el consumo, **cuando** se procesa, **entonces** no se crea un voto parcialmente válido. | EP-07 |
| TS-01 | Proveer wallet y firmar contenido de forma efímera | Como Developer, quiero exponer capacidades de provisión y firma para una wallet individual, para firmar votos sin persistir claves privadas reconstruidas. | **Dado** que llega una solicitud válida de provisión, **cuando** se procesa, **entonces** se crea una capacidad de firma con posición de derivación única y se devuelve su referencia pública. **Dado** que Voting solicita firmar un único contenido con wallet habilitada, **cuando** se procesa, **entonces** se reconstruye la clave, se firma y se olvida dentro de la misma operación. **Dado** que la wallet está suspendida/retirada o ya está abierta para otro contenido, **cuando** recibe la solicitud, **entonces** se rechaza sin producir firma. | EP-08 |
| TS-02 | Entregar votos firmados a blockchain | Como Developer, quiero aceptar contenido ya firmado y publicar su estado de entrega, para que Voting distinga intención, fallo y confirmación on-chain. | **Dado** que la API recibe contenido firmado y referencia de pagador válidos, **cuando** acepta la orden, **entonces** responde con referencia de entrega y la coloca en el orden secuencial del pagador. **Dado** que la red confirma la transacción, **cuando** se observa el resultado, **entonces** se publica una confirmación idempotente contabilizable. **Dado** que el envío falla, **cuando** se agota el único reintento permitido, **entonces** se marca abandonada con su razón sin duplicar el efecto. | EP-08 |
| US-27 | Cerrar y contabilizar una propuesta | Como directiva o administradora, quiere cerrar la votación y obtener el resultado, para documentar el acuerdo de la asamblea. | **Dado** que la propuesta está abierta y se alcanza su condición de cierre, **cuando** se cierra, **entonces** se dejan de aceptar autorizaciones y votos. **Dado** que está cerrada, **cuando** se realiza el conteo, **entonces** se contabilizan solo votos confirmados on-chain, calculando participación y cuórum congelado. **Dado** que existen votos pendientes o fallidos, **cuando** se calcula el resultado, **entonces** no se presentan como confirmados. | EP-07 |
| US-28 | Auditar resultados y evidencia | Como propietario, socio o directiva, quiere revisar el resultado y su evidencia verificable, para comprobar el conteo sin depender únicamente del operador. | **Dado** que una propuesta fue contabilizada, **cuando** se consulta su resultado, **entonces** se presentan conteos por opción, participación, veredicto de cuórum y referencias de confirmación. **Dado** que se verifica una firma y su transacción, **cuando** los datos corresponden a la propuesta y al firmante esperado, **entonces** la evidencia confirma autoría técnica e inclusión. **Dado** que la evidencia no coincide o la tx no está confirmada, **cuando** se verifica, **entonces** no se contabiliza como voto válido. | EP-07 |
| US-16 | Otorgar y revocar consentimiento por alcance | Como propietario o socio, quiere aceptar o retirar consentimientos específicos, para controlar el procesamiento biométrico y las comunicaciones. | **Dado** que revisa un alcance y versión de política, **cuando** otorga consentimiento, **entonces** se registra alcance, versión y fecha sin asumir consentimiento general. **Dado** que existe consentimiento vigente, **cuando** lo revoca, **entonces** se impiden nuevos tratamientos bajo ese alcance sin alterar hechos históricos. **Dado** que un contexto consulta si puede procesar o contactar, **cuando** no existe consentimiento vigente, **entonces** recibe una respuesta negativa comprobable. | EP-05 |

**Lectura arquitectónica de la selección:** el flujo crítico es US-19 → US-20 → US-21 → US-15/US-23 → US-24 → US-25/TS-01 → TS-02 → US-27 → US-28, con US-16 como guarda transversal. Cualquier decisión que rompa la cadena (p. ej. mezclar signer con payer, reescribir snapshots, persistir imágenes) invalida la verificabilidad.
